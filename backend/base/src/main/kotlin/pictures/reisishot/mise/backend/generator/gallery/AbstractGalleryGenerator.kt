...
import pictures.reisishot.mise.backend.generator.pages.SourcePath
...
abstract class AbstractGalleryGenerator(
    ...
    private suspend fun buildCache(
        ...
        if (!recomputeGallery) return@with
    ...
    private suspend fun buildImageInformation(configuration: WebsiteConfiguration) = with(configuration.parser) {
        configuration.inPath.withChild(AbstractThumbnailGenerator.NAME_IMAGE_SUBFOLDER)
            .list()
