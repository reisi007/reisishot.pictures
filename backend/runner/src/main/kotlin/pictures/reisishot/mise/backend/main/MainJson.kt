...
import pictures.reisishot.mise.backend.generator.gallery.JsonGalleryGenerator
...
object MainJson {
    const val folderName = "data.reisishot.pictures"
    ...
    internal fun build(isDevMode: Boolean) {
        ...
        val sourceFolder = Paths.get("input", Main.folderName).toAbsolutePath()

        val testimonialLoader = TestimonialLoaderImpl.fromSourceFolder(sourceFolder)

        val galleryGenerator = JsonGalleryGenerator(
